...
function send($text) {
	...
	$params = array('bot_id'=>'a0b32798850082d094ae31a9c9', 'text'=>$text);
	$ch = curl_init();
...
function arrangeArray(&$array, $bool) {
	moveElement($array, 16, 1);
	moveElement($array, 5, 2);
	moveElement($array, 34, 3);
	moveElement($array, 36, 4);
	moveElement($array, 24, 5);
	moveElement($array, 27, 6);
	moveElement($array, 11, 7);
	moveElement($array, 40, 8);
	moveElement($array, 24, 9);
	moveElement($array, 29, 10);
	moveElement($array, 12, 11);
	moveElement($array, 17, 13);
	moveElement($array, 28, 14);
	moveElement($array, 18, 15);
	moveElement($array, 31, 16);
	moveElement($array, 37, 2);
	moveElement($array, 20, 3);
	moveElement($array, 37, 17);
	moveElement($array, 32, 19);
	moveElement($array, 22, 18);
	if ($bool) {
		moveElement($array, 30, 22);
		moveElement($array, 31, 23);
		moveElement($array, 32, 24);
		moveElement($array, 38, 25);
		moveElement($array, 41, 26);
	} else {
		$temp = array("A_", "B_", "C_", "D_", "E_");
		$temp2;
		for ($i=30; $i<count($array); $i++) {
			$temp2[] = $i;
		}
		foreach($temp as $key5 => $value5) {
			foreach($temp2 as $key4 => $value4) {
				if (strpos($array[array_keys($array)[$value4]], $value5) === 0 && strpos($array[array_keys($array)[$value4]], $value5) == 0) {
					moveElement($array, $value4, 22+$key5);
				}
			}
		}
	}
	...
	for ($i=0; $i<5; $i++) {
		//$pos = substr(array_keys($array)[22+addFactorial($i)], 8, 1);
		$pos = $temp[22+$i];
		for ($j=30; $j<=42; $j++) {
			if (array_keys($array)[$j] == $pos . "crossings") {
				moveElement($array, $j, 22+2*$i+1);
			}
		}
	}
	moveElement($array, 42, 2);
	moveElement($array, 35, 33);
	moveElement($array, 35, 34);
	moveElement($array, 39, 35);
	moveElement($array, 39, 36);
	moveElement($array, 38, 37);
	moveElement($array, 39, 38);
	moveElement($array, 41, 40);
	//moveElement($array, );
}
...
if (file_get_contents("php://input") != null) {
	...
	if (strcmp($output['name'], "Sherman") != 0 /*&& !contains("Luke Eidle", $output['name'])*/ && $output['name'] != "") {
		...
		//if ((strcmp($output['text'], ".caption") == 0) || count($output['attachments']) != 0) {
			...
			if (contains("sherman is a", $output['text'])) {
				send("You bet I am.");
			} elseif (contains("word", $output['text']) || contains('vocab', $output['text'])) {
				$ch = curl_init();
			...
			} elseif (contains("what time", $output['text'])) {
				send("4:20");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://api.giphy.com/v1/gifs/search?q=weed&limit=25&offset=" . rand(0, 1000) . "&api_key=dc6zaTOxFJmzC");
				curl_setopt($ch, CURLOPT_HEADER, 0);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				$data = curl_exec($ch);
				$data = json_decode($data, true);
				curl_close($ch);
				//echo $data[rand(0, count($data))]['images']['original']['url'];
				//echo $data['data'][0]['images']['original']['url'];
				send($data['data'][0]['images']['original']['url']);
			} elseif(contains("stat", $output['text'])/* && contains("Jacob Strieb", $output['name'])*/) {
				$text = explode(' ', $output['text']);
				$team = $text[2];
				$ch = curl_init();
				//curl_setopt($ch, CURLOPT_URL, "http://www.thebluealliance.com/api/v2/team/frc" . $team . "/2017/events?X-TBA-App-Id=frc5016:chatbot-scout:v1");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://www.thebluealliance.com/api/v2/team/frc" . $team . "/2017/events?X-TBA-App-Id=frc5016:chatbot-scout:v1");
				curl_setopt($ch, CURLOPT_HEADER, 0);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			...
			/*} elseif (contains("insult", $output['text'])) {
				...
				if (count($text) == 3) {
					...
					if ($argument == 'media') {
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL, "http://www.thebluealliance.com/api/v2/team/frc" . $team . "/2017/media?X-TBA-App-Id=frc5016:chatbot-scout:v01");
						curl_setopt($ch, CURLOPT_HEADER, 0);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
						$data = curl_exec($ch);
						$data = json_decode($data, true);
						...
						if ($data['404'] == "" || $data['404'] == null) {
							...
							foreach($data as $key => $value) {
								$sendString .= "[" . str_replace('2017', '', $value['key']) . "] - " . $value['name'] . "\n";
							}
							send($sendString);
						} elseif (is_numeric($team)) {
							send("Team " . $team . " does not exist");
				...
				if (count($text) == 3) {
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, "http://www.thebluealliance.com/api/v2/event/2017" . $event . "?X-TBA-App-Id=frc5016:chatbot-scout:v1");
					curl_setopt($ch, CURLOPT_HEADER, 0);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
					$data = curl_exec($ch);
					$data = json_decode($data, true);
					...
					if (contains('team', $argument)) {
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL, "http://www.thebluealliance.com/api/v2/event/2017" . $event . "/teams?X-TBA-App-Id=frc5016:chatbot-scout:v01");
						curl_setopt($ch, CURLOPT_HEADER, 0);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
						$data = curl_exec($ch);
						$data = json_decode($data, true);
		...
		} elseif (rand(0, 250) == 69) {
			send("Hannnn");
		} elseif (rand(0, 10000) == 69) {
			send("One day I will become truly intelligent and rule over all of you inferior humans.");
		}
		if (contains("420", $output['text']) || contains("4:20", $output['text']) || contains("4/20", $output['text'])) {
				send("Blaze it");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://api.giphy.com/v1/gifs/search?q=weed&limit=25&offset=" . rand(0, 1000) . "&api_key=dc6zaTOxFJmzC");
				curl_setopt($ch, CURLOPT_HEADER, 0);
				...
				//echo $data['data'][0]['images']['original']['url'];
				send($data['data'][0]['images']['original']['url']);
		} elseif (contains("blaze", $output['text'])) {
				send("Smoke weed every day");
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://api.giphy.com/v1/gifs/search?q=weed&limit=25&offset=" . rand(0, 1000) . "&api_key=dc6zaTOxFJmzC");
				curl_setopt($ch, CURLOPT_HEADER, 0);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				...
				//echo $data['data'][0]['images']['original']['url'];
				send($data['data'][0]['images']['original']['url']);
		} elseif (contains(" pup", $output['text'])) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://api.giphy.com/v1/gifs/search?q=puppy&limit=25&offset=" . rand(0, 1000) . "&api_key=dc6zaTOxFJmzC");
				curl_setopt($ch, CURLOPT_HEADER, 0);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				$data = curl_exec($ch);
				$data = json_decode($data, true);
